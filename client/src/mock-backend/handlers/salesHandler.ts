...
const nextId = () => 
  sales.length ? Math.max(...sales.map(s => s.id)) + 1 : 1
...
const calculateSaleTotals = (saleItems: SaleItem[]): { total_cost: Money, total_revenue: Money, total_profit: Money } => {
  const totalCost = saleItems.reduce((sum, item) => sum + (item.cost.amount * item.quantity), 0)
  const totalRevenue = saleItems.reduce((sum, item) => sum + item.revenue.amount, 0)
  const totalProfit = saleItems.reduce((sum, item) => sum + item.profit.amount, 0)
...
export const salesHandlers = [
  ...
  http.post(url(api.sales.store), async ({ request }) => {
    ...
    const { date, time } = body.sold_at ? 
      { date: body.sold_at.split('T')[0], time: body.sold_at.split('T')[1]?.split('.')[0] || '00:00:00' } : 
      getTodayDateTime()
  ...
  // GET /sales/:id
  http.get(
    url(api.sales.show(0)).replace('/0', '/:id'),
    async ({ params }) => {
